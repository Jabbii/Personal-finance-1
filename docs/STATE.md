...
- **HEIC conversion is planned but unnecessary** — a scan of all 10 OneDrive bank folders on 2026-08-23 found 1,956 files and **zero HEIC**: 1,219 `.jpg`, 565 `.jpeg`, 172 `.png`. The golden set matches (jpg/png/jpeg/pdf only). The plan's `heic-convert` dependency and Chunk 2.1 conversion sub-step came from an iPhone assumption; the user is on Samsung, and these are screenshots, which Android never saves as HEIC. Nothing is installed yet, so this is plan-level only. **Proposal for Chunk 2.1:** drop `heic-convert`, and instead have `sync.js` route any unrecognised extension to the failure queue with a push notification, so a future phone or camera-setting change surfaces loudly rather than silently dropping transactions.
- **Krungthai NEXT is effectively untested** — that OneDrive folder holds exactly 1 file, against dozens-to-hundreds for every other bank, and the `fixtures/krungthai/` golden set is built almost entirely from PaoTang screenshots. May be accurate if the user rarely uses the Krungthai NEXT app directly, but the bank has no meaningful eval coverage.
